...
from validation_scheme import CreateUser, UpdateUser
...
def validate(json_data, validation_scheme):
    try:
        ...
        error_message = json.loads(err.json())  # сделаем сообщение более читаемым
        raise HttpError(400, error_message)
    

def hash_password(password: str):
    password = str(password).encode()  # переводим в байты
    password = md5(password).hexdigest()  # хэшируем
    return password
...
app.add_url_rule(
    "/advertisements/<int:adv_id>/", view_func=advertisement_view, methods=["GET", "PATCH", "DELETE"]
)
